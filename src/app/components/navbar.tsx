...
export default function Navbar({isProfile=false}:NavbarProps){
        ...
        <nav className="flex w-screen h-[65px] bg-customBlack justify-between py-[5px] px-[20px] items-center p-5">
            {
                isProfile ? (
                    <>                
                    <button className="flex items-center gap-1 w-fit text-white" onClick={handleBack}>Volver</button>
                    <button className="flex items-center gap-1 w-fit text-white" onClick={handleSession}><IconLogoutBoxLine/>Cerrar Sesión</button>
                    </>
                ):(
                <>
